...
sandbox/                   # Toolchain image (Containerfile, build.sh), wtx per-worktree sandboxes + in-sandbox services (non-stow; wtx is stowed) → sandbox/README.md
...
| Tests                          | `pi/.pi/agent/extensions/**/*.test.ts`, `pi/.local/bin/tdd-plan.test.ts`, `pi/.local/bin/lib/`, `obsidian/.local/lib/wiki-search/wiki-search.test.ts`, `obsidian/.local/lib/wiki-core/`, `obsidian/.local/lib/issue/`; plus local-only plenary Lua suite in `tests/` (`tests/run.sh`) |
