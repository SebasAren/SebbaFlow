...
- **Pre-commit checks**: run `mise run pre-commit` (format + lint + typecheck + tests). The version-controlled hook at `.githooks/pre-commit` invokes it via `core.hooksPath`.
