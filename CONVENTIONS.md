...
- **Pre-commit checks**: run `mise run pre-commit` (format + lint + typecheck + tests). The version-controlled hook at `.githooks/pre-commit` invokes it via `core.hooksPath`; a shell wrapper in `bashrc/.bashrc.d/alias` intercepts `jj commit`/`jj ci`.
- **Commit messages**: optionally generated by pi via the commit skill
